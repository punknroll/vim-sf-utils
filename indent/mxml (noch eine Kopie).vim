...
function! <SID>MxmlIndentOpen(lnum, pattern)
    ...
    let s = substitute(s, '\C</fx:', "\2", 'g')
    let s = substitute(s, '\C</s:', "\2", 'g')
...
function! <SID>MxmlIndentClose(lnum, pattern)
    ...
    let multi_closing = substitute(s, '\C<\/fx:', "\1", 'g')
    let multi_closing = substitute(s, '\C<\/s:', "\1", 'g')
...
function! <SID>MxmlIndentSum(lnum, style)
	...
	if a:style == match(currline, '\C^\s*</\<\(s:\)')
	if a:style == match(currline, '\C^\s*</\<\(fx:\)')
...
function! MxmlIndentGet(lnum)
    ...
    let as_end   = 0
    " Move the cursor to the start
    let as_start = search('\C<fx:Script',   'Wb')
    if as_start != 0
        " If a match was found, find the end to see
        " if the end tag is before our current line
        let as_end   = search('\C</fx:Script>', 'nW' )  
    endif
